...
// Pending queue in FCFS order
static std::list<SchedJob*> *pending = nullptr;

// For each running job, which hosts it has and when it’s predicted to end
static std::unordered_map<std::string, std::set<uint32_t>> allocations;
static std::unordered_map<std::string, double>            end_times;

// Free hosts right now
static std::set<uint32_t> available_hosts;
static uint32_t           platform_nb_hosts = 0;

// === Helpers ===
// Given the head‐job’s host count, find the earliest time ≥ now when ≥ that many
// hosts will be free again (using end_times).
static double compute_reservation(double now, uint32_t need_hosts) {
    // If we already have enough free, we reserve _now_.
    uint32_t free_now = available_hosts.size();
    if (free_now >= need_hosts) return now;

    // Build a list of (end_time, freed_hosts) and sort by end_time
    std::vector<std::pair<double,uint32_t>> events;
    events.reserve(end_times.size());
    for (auto &kv : end_times) {
        const auto &jid = kv.first;
        double t_end = kv.second;
        uint32_t q   = allocations[jid].size();
        events.emplace_back(t_end, q);
    }
    std::sort(events.begin(), events.end(),
              [](auto &a, auto &b){ return a.first < b.first; });

    // Scan until we accumulate enough freed hosts
    for (auto &ev : events) {
        free_now += ev.second;
        if (free_now >= need_hosts)
            return ev.first;
    }
    // In pathological case never enough, fall back to the _last_ end‐time
    return events.empty() ? now : events.back().first;
}

// Allocate exactly `q` hosts from available_hosts (the lowest‐numbered),
// record them in `allocations[job_id]`, and remove from available_hosts.
static std::string allocate_hosts(const std::string &job_id, uint32_t q) {
    ...
    for (auto h : picked) available_hosts.erase(h);
...
extern "C" uint8_t batsim_edc_init(const uint8_t *data,
                                   uint32_t size,
                                   uint32_t flags)
{
    (void)data; (void)size;
    ...
    uint32_t *decisions_size)
{
    ...
    for (auto *ev : *msg->events()) {
        switch (ev->event_type()) {
          case fb::Event_BatsimHelloEvent:
            mb->add_edc_hello("easy-backfill","1.0.0");
            break;
          ...
          case fb::Event_JobSubmittedEvent: {
            ...
            j->walltime = s->job()->walltime();  // user‐provided p̃j
            // Reject if they asked for too many hosts
    ...
    // ---- 2) EASY‑FCFS‑FCFS loop ----
    bool progress = true;
    while (progress && !pending->empty()) {
      progress = false;

      // 2a) Try the head job in strict FCFS
      ...
      // 2b) Cannot run head: compute its reservation horizon
      double reserve_t = compute_reservation(now, head->nb_hosts);

      // 2c) Backfill any job that (a) fits available slots and (b) finishes before reserve_t
      for (auto it = std::next(pending->begin()); it != pending->end(); /*nothing*/) {
        SchedJob *cand = *it;
    ...
    // ---- 3) Send out the batch of execute/reject messages ----
