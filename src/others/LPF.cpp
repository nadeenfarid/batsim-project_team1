  ...
  uint32_t * decisions_size)
{
  ...
  for (unsigned int i = 0; i < nb_events; ++i) {
    ...
    switch (event->event_type()) {
      ...
      case fb::Event_JobSubmittedEvent: {
        ...
        job->estimated_time = parsed_job->job()->walltime(); //for LPF
  ...
  // Find the job with the longest estimated_time
  auto longest_job_it = std::max_element(jobs->begin(), jobs->end(),
    [](const SchedJob* a, const SchedJob* b) {
      return a->estimated_time < b->estimated_time;
    });

  currently_running_job = *longest_job_it;
  jobs->erase(longest_job_it); // Remove from list
