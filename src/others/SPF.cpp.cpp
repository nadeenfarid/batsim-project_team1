...
struct SchedJob {
  std::string job_id;
  uint8_t nb_hosts;
  uint64_t estimated_time; // for adding time taken to the criteria of choosing tasks 
};
  ...
  uint32_t * decisions_size)
{
  ...
  for (unsigned int i = 0; i < nb_events; ++i) {
    ...
    switch (event->event_type()) {
      ...
      case fb::Event_JobSubmittedEvent: {
        ...
        job->estimated_time = parsed_job->job()->walltime(); // <- important for SPT
  ...
  // Find the job with the shortest estimated_time
  auto shortest_job_it = std::min_element(jobs->begin(), jobs->end(),
    [](const SchedJob* a, const SchedJob* b) {
      return a->estimated_time < b->estimated_time;
    });

  currently_running_job = *shortest_job_it;
  jobs->erase(shortest_job_it); // Remove from job list
