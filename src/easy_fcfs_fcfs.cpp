    ...
    uint32_t *decisions_size)
{
    ...
    for (auto *ev : *msg->events()) {
        switch (ev->event_type()) {
            case fb::Event_BatsimHelloEvent:
                mb->add_edc_hello("easy-fcfs", "1.0.0");
                break;
            ...
            case fb::Event_JobSubmittedEvent: {
                ...
                j->submit_time = now;  // for FCFS tracking
    ...
    while (progress && !pending->empty()) {
        ...
        // No sorting = FCFS: just use list order
        ...
        for (auto it = std::next(pending->begin()); it != pending->end(); /*++it handled*/) {
            SchedJob *cand = *it;
